...
const ShowListComment: React.FC<ShowListCommentProps> = ({ dataId, type }) => {
  ...
  const handleShowReplies = async (ratingId: string) => {
    if (!replies[ratingId]) {
      const data = { ratingId, pageNumber: 1, pageSize: 5 };
      await getCommentReply(data, dispatch);
    }
  };

  const handleReply = async (formData: FormData) => {
    // Extract content and images from formData
    const content = formData.get('content') as string;
    const images: File[] = formData.getAll('images') as File[]; // Assuming 'images' is the name used in your form
  
    // Implement logic to send reply to the server here
    console.log("Reply Content: ", content);
    console.log("Attached Images: ", images);
  
    // Reset replyTo after submitting the reply
    setReplyTo(null);
  };
          ...
          <article key={rating.ratingId} className="px-6 pt-6 pb-2 text-base bg-white border-b border-gray-200 dark:border-gray-700 dark:bg-gray-900">
            ...
            <div className="flex items-center mt-4 space-x-4">
              ...
              >
                ...
                {replies[rating.ratingId] ? 'Ẩn bình luận' : 'Xem bình luận'}
              </button>
            </div>
