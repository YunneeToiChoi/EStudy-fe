...
export default function GetDocByUser() {
    ...
    const handleSearch = debounce((query: string) => {
      setSearchQuery(query); // Update search query here
    }, 300); // 300ms delay
    ...
    // Update filtered documents based on category, course, and search query
    useEffect(() => {
      const filtered = listDocuments?.filter((doc: any) => 
    ...
    }, [selectedCategory, selectedCourse, listDocuments, searchQuery]);
    ...
    return (
      <div className="container mx-auto px-4 flex flex-col">
        ...
        {/* Bộ lọc tài liệu */}
        <div className="flex flex-wrap gap-4 mb-6 items-center mt-40">
          <select 
            className="border border-gray-300 rounded-md py-2 px-4 w-full max-w-xs transition-all duration-300 shadow-sm focus:outline-none focus:ring focus:ring-blue-400 overflow-hidden whitespace-nowrap text-ellipsis" 
            value={selectedCourse || ''}
            onChange={(e) => setSelectedCourse(e.target.value ? String(e.target.value) : null)}
          >
          ...
          <select 
            className="border border-gray-300 rounded-md py-2 px-4 w-full max-w-xs transition-all duration-300 shadow-sm focus:outline-none focus:ring focus:ring-blue-400" 
            value={selectedCategory || ''}
            onChange={(e) => setSelectedCategory(e.target.value ? String(e.target.value) : null)}
          >
